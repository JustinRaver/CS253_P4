//this file defines the functino executeCommand(char *str)
...
void executeCommand(char *str){
    ...
    if(strncmp(args[0],"exit",20) == 0){//if arg is 'exit'
        ...
        exitStatus =127;//exit abnormally to show functionality has not yet been added
        add_history(argsCpy,exitStatus);//add cmd
        int i;
        for(i=0;i<argTracker;i++){//print the external commands tokenized
            printf("[%d", i);
            printf("] %s\n",args[i]);
        }
    }
